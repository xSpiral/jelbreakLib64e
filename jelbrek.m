...
//    (char **) malloc(sizeof(char *) * (count + 1));
    ...
    int i;
    for(i = 0; i < count; i++) {
        NSString *s = [paths objectAtIndex:i];//get a NSString
    ...
    inject_trusts([paths count], cargs);
    
    
//    uint64_t trust_chain = 0xFFFFFFF008F702C8 + KASLR_Slide;
//
////    uint64_t trust_chain = KernelBase + (0xFFFFFFF008F702C8 - 0xFFFFFFF007004000);
//
...
//    printf("[*] step 7\n");
//
//    //uint64_t f_load_trust_cache = KernelBase + (0xFFFFFFF007B80504 - 0xFFFFFFF007004000);
//
//    uint32_t ret = kernel_call_7(0xFFFFFFF007B80504 + KASLR_Slide, 3,
//                                  kernel_trust,
//                                  length,
//                                  0);
//
...
int launchAsPlatform(char *binary, char *arg1, char *arg2, char *arg3, char *arg4, char *arg5, char *arg6, char**env) {
    ...
    printf("we made it to 1\n");
    
    int rv = posix_spawn(&pd, binary, NULL, &attr, (char **)&args, env);
    if (rv) return rv;
    
    printf("we made it to 2\n");
    
    platformize(pd);
    
    printf("we made it to 3\n");
    
    kill(pd, SIGCONT); //continue
    
    printf("we made it to 4\n");
